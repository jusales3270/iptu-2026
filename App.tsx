...
const BOITUVA_COAT_OF_ARMS_URL = "https://upload.wikimedia.org/wikipedia/commons/thumb/7/77/Bras%C3%A3o_de_Boituva.svg/512px-Bras%C3%A3o_de_Boituva.svg.png";
...
export default function App() {
  ...
  const handleSendMessage = useCallback(async (params: { input: string; attachments: Attachment[] }) => {
    ...
    try {
      const stream = streamGeminiResponse(messages, input, newAttachments);
      ...
      setMessages((prev) =>
        prev.map((msg) =>
          msg.id === botMessageId
            ? { ...msg, content: '**Erro:** Algo deu errado. Por favor, tente novamente.', isThinking: false }
            : msg
        )
      );
    } finally {
